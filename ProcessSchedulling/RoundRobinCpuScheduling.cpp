...
// Video Explanation:
// https://www.youtube.com/watch?v=8uGblP-ag3o&list=PLVKIC9j3jSYtCEAffM_51qVWeQus70cqc&index=10
...
int main() {
  ...
  while (completed != n) {
    ...
    if (ps[index].bt_remaining == ps[index].bt) {
      ...
      total_idle_time +=
          (is_first_process == true) ? 0 : ps[index].start_time - current_time;
    ...
    // check which new Processes needs to be pushed to Ready Queue from Input
    // list
    for (int i = 1; i < n; i++) {
    ...
    // if queue is empty, just add one process from list, whose remaining burst
    // time > 0
    if (q.empty()) {
      for (int i = 1; i < n; i++) {
