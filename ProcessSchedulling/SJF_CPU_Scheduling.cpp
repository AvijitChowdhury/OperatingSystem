/*.................C++ Program to Implement SJF ( Shortest Job First) CPU
SCheduling Algorithm..................... SJF - A Non Preemptive Algorithm This
Program works for same as well as different arrival times
...
int main() {
  ...
  while (completed != n) // all n processes are not completed yet
  {
    // find process with min. burst time in ready queue at current time
    int min_index =
        -1; // min index is the index which proces having least burst time
    int minimum = INT_MAX; // minimum is the number of least burst time
    for (int i = 0; i < n; i++) {
    ...
    if (min_index == -1) {
      ...
      total_idle_time +=
          (is_first_process == true) ? 0 : (ps[min_index].start_time - prev);
